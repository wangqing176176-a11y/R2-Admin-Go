...
- 密码保护、指定成员、指定成员 + 密码三种模式
- 按团队角色或具体成员授权
- 可向无权成员隐藏文件夹，列表、搜索、收藏和回收站同步过滤
- 密码模式中，未输入密码的授权成员仍可看到文件夹并解锁；隐藏仅针对身份无权的成员
- 创建者和当前设置者保留访问资格，管理员管理策略不自动绕过内容访问限制
- 保留现有密码；新密码支持 8–128 位字符，密码输入留空可保留旧密码
- 解锁授权绑定成员、团队、策略版本和有效期，修改策略后旧授权失效
- 受保护文件通过本地代理访问，重新核验当前权限；旧公开地址和已签发的 R2 直链不在应用的撤销范围内
- 移动、复制及恢复文件夹时保留保护；回收期间原路径的保护继续生效
- 当前不支持嵌套配置；文件夹策略只能收紧已有 RBAC 权限

这是应用层访问控制，不会对 R2 中的文件内容进行重新加密。私有资料应使用未开放公共访问的存储桶。
...
启用混合文件夹保护前，执行 `supabase/folder_access_policies.sql`（先完成上面的六个初始化脚本）。
这是可重复执行的增量升级，保留已有密码，新增访问策略字段和数据库级密码尝试限流。
升级后路径、密码哈希和授权配置仅经服务端读取，避免直接查询 Supabase 绕过文件夹保护。

权限回归检查：`npm run test:folder-access`。
