...
const ModalChrome = React.memo(function ModalChrome({ title, children, onClose, footer, contentClassName, headerRight, showHeaderClose, busy }: ModalChromeProps) {
  return <>
    <div className="r2-modal-header relative border-b border-gray-100 bg-white px-5 py-4 dark:border-gray-800 dark:bg-gray-900">
      {headerRight ? (
        <div className={`absolute ${showHeaderClose ? "right-14 top-1/2 h-8 -translate-y-1/2" : "right-3 top-1/2 -translate-y-1/2"} inline-flex items-center`}>
          {headerRight}
        </div>
      ) : null}
      {showHeaderClose ? (
        <button type="button" onClick={onClose} aria-label="关闭弹窗" disabled={busy}
...
export default function Modal({
  ...
  useLayoutEffect(() => {
    ...
    // Preserve the panel's current layout height while its contents animate out.
    const measure = () => { panel.style.setProperty("--r2-open-height", `${panel.offsetHeight}px`); };
    measure();
