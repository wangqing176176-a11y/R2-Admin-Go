...
import { Check, Code2, Copy, Eye, ExternalLink, Image as ImageIcon, List } from "lucide-react";
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950">
      ...
      {isLoading ? <div className="flex min-h-0 flex-1 items-center justify-center bg-[#fbfcfe] text-sm text-slate-500 dark:bg-[#0b1020] dark:text-slate-400">文本加载中…</div> : isMarkdown && viewMode === "preview" ? (
        <div className="flex min-h-0 flex-1 bg-[#fbfcfe] dark:bg-[#0b1020]">
          {outlineOpen ? <aside className="hidden w-56 shrink-0 overflow-y-auto border-r border-slate-200 bg-white/80 p-3 md:block dark:border-slate-800 dark:bg-slate-950/70"><div className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300"><List className="h-3.5 w-3.5" />文档大纲</div><nav className="space-y-0.5">{headings.map((heading) => <button type="button" key={heading.id} onClick={() => scrollToHeading(heading)} className="block w-full rounded px-2 py-1.5 text-left text-xs leading-5 text-slate-500 transition hover:bg-blue-50 hover:text-blue-700 dark:text-slate-400 dark:hover:bg-blue-950/40 dark:hover:text-blue-300" style={{ paddingLeft: `${8 + Math.max(0, heading.level - 1) * 10}px` }}>{heading.text}</button>)}</nav></aside> : null}
